...
@njit
def mcloop_alg300a(Qmax, cbp, sbp, S, xinda, yinda, zinda):
        ...
        for R in range(S_sz0):  # along axis 1, along row
            for C in range(S_sz1):  # along axis 0, along column.
                ...
                if Neigh.min() != Neigh.max():
                    ...
                    '''DelH1 = np.sum(Neigh == ssub_111_a) - 1'''
                    DelH1 = 0
                    for n in Neigh:
                        if n == ssub_111_a:
                            DelH1 += 1
                    DelH1 -= 1  # Exclude self-match
                    # ---------------------------------------------
                    '''ssub_111_b = np.random.choice(Neigh, 1)[0]'''
                    # ssub_111_b = Neigh[np.random.randint(0, 27)]
                    ssub_111_b = np.random.randint(1, Qmax)
                    # ---------------------------------------------
                    ...
                    '''DelH2 = np.sum(Neigh == ssub_111_b) - 1'''
                    DelH2 = 0
                    for n in Neigh:
                        if n == ssub_111_b:
                            DelH2 += 1
                    DelH2 -= 1  # Exclude self-match
                    # ---------------------------------------------
                    if DelH2 >= DelH1:
                        _p = zinda[P1, R1, C1]
                        _r = yinda[P1, R1, C1]
...
def mc_iterations_3d_alg300a(S=None,
    ...
    for m in range(uisim.mcsteps):
        if S.min() == S.max():
            ...
            S = mcloop_alg300a(uisim.S,
                              cbp, sbp, S, xinda, yinda, zinda)
