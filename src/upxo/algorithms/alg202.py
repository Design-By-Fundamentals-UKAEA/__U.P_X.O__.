    ...
    for s0 in range(S.shape[0]):  # along axis 0
        ...
        for s1 in range(S.shape[1]):  # along axis 1
            ...
            if Neigh.min() != Neigh.max():
                ...
                DelH1 = np.sum(Neigh == ssub_11) - 1

                Neigh = Neigh[Neigh != ssub_11]
                random_index = np.random.randint(0, len(Neigh))
                ssub_11_b = Neigh[random_index]

                DelH2 = np.sum(Neigh == ssub_11_b)
...
def run(uisim, uiint, uidata, uigrid,
    ...
    for m in range(uisim.mcsteps):
        if S.min() == S.max():
            ...
            S = mcloop_alg202(cbp, sbp, S, AIA0, AIA1)
        # ------------------------------------------------
...
def run(uisim, uiint, uidata, uigrid,
    ...
    for m in range(uisim.mcsteps):
        if S.min() == S.max():
            ...
            sbp = uisim.s_boltz_prob
            # --------------------------------
            #def numba_optimized_loop(S, AIA0, AIA1, NLM, xgr, ygr):
            # --------------------------------
            for s0 in range(S.shape[0]):  # along axis 0
                s00, s01, s02 = s0+0, s0+1, s0+2
                for s1 in range(S.shape[1]):  # along axis 1
                    ...
                    if min(Neigh) != max(Neigh):
                        ...
                        random_index = random.randint(0, len(Neigh) - 1)
                        ssub_11_b = Neigh[random_index]
                        ...
                        if DelH2 >= DelH1:
                            ...
                            if sbp[int(ssub_11_b-1)] < random.random():
                                S[s0, s1] = ssub_11_b
        cond_1 = m % uiint.mcint_save_at_mcstep_interval == 0.0
        save_msg = False
        if m==0 or cond_1 or fully_annealed:
