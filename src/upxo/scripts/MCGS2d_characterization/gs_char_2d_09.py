...
'''geom.are_grains_closed_usenodes()'''
[gb.check_closed() for gb in GB.values()]

'''geom.are_grains_closed_usecoords()'''
flags = []
for gid in geom.gid:
    flag = np.abs(GBCoords[gid][0] - GBCoords[gid][-1]).sum() <= geom.EPS_coord_coincide
    flags.append(flag)
...
for i, seg in enumerate(GB[gid].segments[1:], start=1):
    if GB[gid].segflips[i]:
...
if force_close:
    coords = GB[gid].force_close_coordinates(coords, assess_first=True)
