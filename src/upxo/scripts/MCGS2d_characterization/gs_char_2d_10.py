...
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
def set_polygonization_xyoffset(xyoffset):
    # Validations
...
def extract_GBP(self):
    ...
    self.GBP = np.array(GBP) - self.xyoffset
# End
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
...
def find_GBP_at_boundary(self):
    ...
    GBP_at_boundary = np.unique(np.vstack((GBP_left, GBP_bot,
    ...
    for i, gbpboundary in enumerate(GBP_at_boundary):
        x, y = gbpboundary
        if not any((self.JNP[:, 0] == x) & (xy[:, 1] == y)):
...
def build_jnp_grain_wise_indices(self):
    ...
    for gid in geom.gid:
        ...
        pol_shapely = affinity.translate(self.polygons[gid-1],
                                         xoff=-self.xyoffset,
                                         yoff=-self.xyoffset)
...
def build_gbmullines_grain_wise(self):
    ...
    for gid in self.gid:
        ...
        jnps_to_insert = jnp_all_upxo[self.jnp_grain_wise_indices[gid]][jnp_indices_toinsert]
        # --------------------------
...
def arrange_junction_point_coords_new(self, gbcoords_thisgrain, junction_points_coord):
    # Create a dictionary to map coordinates to their indices in gbcoords_thisgrain
    coord_index_map = {tuple(coord): idx for idx, coord in enumerate(self.gbcoords_thisgrain)}
    # Generate a list of indices for sorting
    sorted_indices = sorted(range(len(self.junction_points_coord)), key=lambda i: coord_index_map[tuple(self.junction_points_coord[i])])
    # Sort junction_points_coord based on the sorted indices
    self.junction_points_coord = np.array([junction_points_coord[i] for i in sorted_indices])
...
def build_sorted_jnp_objects(self, plot=False):
    ...
    for gid in self.gid:
        self.gbpoints_thisgrain = self.gbmullines_grain_wise[gid].nodes
        self.gbcoords_thisgrain = self.gbmullines_grain_wise[gid].get_node_coords()
        junction_points_upxo, junction_points_coord = [], []
        for i, jnpcoord in enumerate(self.jnp_all_coords):
            if DO.is_a_in_b(jnpcoord, self.gbcoords_thisgrain):
                self.junction_points_coord.append(jnpcoord)
                self.junction_points_upxo.append(jnp_all_upxo[i])
        ...
        self.junction_points_upxo = list(np.array(self.junction_points_upxo)[_])
        # junction_points_upxo, _ = arrange_junction_points_upxo(gbpoints_thisgrain, junction_points_upxo)
        self.jnp_all_sorted_coords[gid] = self.junction_points_coord
        self.jnp_all_sorted_upxo[gid] = self.junction_points_upxo
...
"""
def splice_grain_boundary_segments_at_junction_points(plot=False):
...
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
def create_neigh_gid_pair_ids(neigh_gid):
    ...
    self.pair_ids_unique_rl = np.flip(self.gid_pair_ids_unique_lr, axis=1)
    # return gid_pair_ids, self.gid_pair_ids_unique_lr, self.pair_ids_unique_rl
# End
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
geom.neigh_gid
    ...
    for i, pair in enumerate(self.gid_pair_ids_unique_lr):
        ...
        for gbseg1 in self.gbsegments[pair[0]]:
            ...
            for gbseg2 in self.gbsegments[pair[1]]:
                ...
                # ----------------------------
                self.nconn[tuple(pair)]['n'].append(len(nconn[tuple(pair)]['gbseg']))
...
def gather_grain_boundary_segments_of_all_pairs(self):
    ...
    for pair in self.gid_pair_ids_unique_lr:
        ...
        if len(_gbseg_) > 0:
            ...
            ###########################################
            self.GBSEGMENTS[tuple(pair)] = [nconn[tuple(pair)]['gbseg'][_ui_] for _ui_ in ui]
            '''As we no longer need any duplicates, we will override the repeated
            ones in ncoon as well.'''
            self.nconn[tuple(pair)]['gbseg'] = [nconn[tuple(pair)]['gbseg'][_ui_] for _ui_ in ui]
            ###########################################
...
def update_consolidated_segments_with_boundary_grain_gids(self, plot=False):
    ...
    for i, seg in enumerate(gbsegments[self.grain_loc_ids['bottom_right_corner']]):
    ...
    for i, seg in enumerate(gbsegments[self.grain_loc_ids['top_right_corner']]):
        if seg.has_coord([xmax, ymax]):
            seg = gbsegments[self.grain_loc_ids['top_right_corner']][i]
            self.consolidated_segments[self.grain_loc_ids['top_right_corner']].append(seg)

    for i, seg in enumerate(gbsegments[self.grain_loc_ids['top_left_corner']]):
        if seg.has_coord([xmin, ymax]):
            seg = gbsegments[self.grain_loc_ids['top_left_corner']][i]
...
def flip_segments_to_reorder_GBS(self, plot_each_grain_details=False):
    ...
    for gid in geom.gid:
        ...
        if not continuity:
            ...
            assessment.'''
            segstart_flip, self.GB[gid] = False, ring2d([gbsegs[0]], [0],
                                                        [segstart_flip])
            '''
            GB[gid].segments
            GB[gid].segids
...
def recalculate_grain_boundary_coordinates_after_gbseg_reordering(self):
...
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
def plot_reordered_GBCoords(self, gid):
    # gid = 43
    # GB[gid].segments
    # GB[gid].segflips
    ...
    if dtype == 'shapely':
        ...
        if throw:
            ...
            if saa:
                ...
                POLYXTAL = MultiPolygon(self.GRAINS.values())
    if plot_polyxtal:
...
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
def AssembleGBSEGS(self, saa=True, throw=False):
    mids_all_gbsegs = []
    for gb in GB.values():
    ...
    if throw:
        return self.mids_all_gbsegs, self.sgseg_obj_list
# End
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
...
def smooth_gbsegs(self, GB, npasses=2, max_smooth_levels=[3, 3], plot=True):
    ...
    all_mids, sgseg_list = self.AssembleGBSEGS(GB_smooth)
    ...
    return GB_smooth, GBCoords_smoothed
# End
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ## ==
# Start
