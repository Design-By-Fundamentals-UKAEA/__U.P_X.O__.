...
class mulpoint2d():
    ...
    def __init__(self,
        ...
        if method in dth.opt.upxo_point2d:
            ...
            m2 = mulpoint2d(method='xlist_ylist',
                            coordx=[10, 12],
                            coordy=[20, 24])
            mulpoint_objects = [m1, m2]
            m3 = mulpoint2d(method='mp', mulpoint_objects=mulpoint_objects)
            ...
            if self.points is None:
                if make_point_objects:
                    self.points = [point2d(x=__x, y=__y,
                                           ...
                                           set_phase=True, phid=1,
                                           phname='ukaea',
                                           set_tcname=True, tcname='B',
                                           set_ea=True,
                                           earepr='Bunge', ea=[45, 35, 0],
    ...
    def __repr__(self, recompute_flag=False):
        ...
        str2 = f' Centroid:{round(self.centroid[0], 6), round(self.centroid[1], 6)}'
        return str1 + str2
    # -----------------------------------------------
    def __len__(self):
        '''
    ...
    def pop(self, remove_indices):
        self.points = [i for j, i in enumerate(self.points) if j not in remove_indices]
        self.recompute_basics(npoints_flag = True, locx_locy_flag = True, centroid_flag = True,)
    # -----------------------------------------------
    def __pow__(self, power):
        ...
        if self.lean in ('high', 'veryhigh'):
            self.locx **= power
            self.locy **= power
        self.recompute_basics(npoints_flag = False, locx_locy_flag = False, centroid_flag = True,)
    # -----------------------------------------------------
    def makegrid(self,
                 method = 'random',
        ...
        if method == 'recgrid':
            self.__grid_rec(xbound, ybound, char_length, space, throw = False)
        elif method == 'hexgrid':
            pass
        elif method == 'trigrid1':
            self.__grid_tri(method, angles, xbound, ybound, char_length, space, throw = False)
        elif method == 'trigrid2':
            self.__grid_tri(method, angles, xbound, ybound, char_length, space, False)
        elif method == 'random':
        ...
        if self.lean in ('no', 'low', 'medium'):
            self.points = deque(point2d(x=_x, y=_y) for (_x, _y) in zip(self.locx, self.locy))
        elif self.lean in ('high', 'veryhigh'):
            self.points = None
    # -----------------------------------------------------
    def __grid_rec(self, xbound, ybound, char_length, space, throw):
        _x = np.linspace(xbound[0], xbound[1], int(abs(xbound[1]-xbound[0])/char_length[0]))
        _y = np.linspace(ybound[0], ybound[1], int(abs(ybound[1]-ybound[0])/char_length[1]))
        if space == 'linear':
    ...
    # -----------------------------------------------------
    def __grid_tri(self, method, angles, xbound, ybound, char_length, space, throw = False):
        '''
        ...
        if method == 'trigrid1':
            ...
            ncopies = int((ybound[1]-ybound[0])/(0*char_length[1]+2*char_length[1]*np.sin(np.radians(angles[1]))))
            __k = 2*char_length[1]*np.sin(_rad(angles[1])) # Just a constant to avoid being calculated in
    ...
    def __grid_rnd(self,
        ...
        if sampling_technique == 'uniform':
            ...
            if randuni_calc == 'by_char_length':
                ...
                _distances = set(distance_matrix(point_coords, point_coords).ravel())
                _distances.remove(0.0) # INDLCUE EXCEPTION HANDLING HERE FOR VERY VERY VERY SMALL VALUES
    ...
    def is_within(self,
        ...
        if input_value>=lowerbound and input_value<=upperbound:
            ...
            return False
    # -----------------------------------------------------
    def assign_rid(self, flag = False, idlength = 4):
        '''
        ...
        if flag:
            try:
                ...
                # string = string.ascii_uppercase + string.ascii_lowercase + string.digits + string.punctuation
                string = ''.join([string.ascii_uppercase,
                                  string.ascii_lowercase,
                                  string.digits,
                                  string.punctuation])
                self.rid = ''.join(random.choice(string) for _ in range(idlength))
        else:
            pass
    # -----------------------------------------------------
    ...
    def unpack(self):
        ...
        being able to store many multi-point objects. In which case, this should
        return a single list of all points across all its point constituent objects.
        NOTE: "point constituent objects" could be either point2d objects or
               mulpoint2d objects

        DEVELOPMENT DECISION @ 12 October 2022:
            It may not be necessary to do this!, given the below restriction to be imposed
        ...
        return self.points
    # -----------------------------------------------
    def add_points(self, toadd, recomp = True):
        '''
        OPERATION:
    ...
    def partition(self,
        ...
        pass
    # -----------------------------------------------
    def neg(self, axis = 'x', indices = None):
        '''
        OPERATION:
            Perform -self.locx
        INPUTS:
        ...
        if self.lean in ('no', 'low', 'medium'):
            ...
            if axis == 'x':
                for i in _iterable: self.points[i].negx()
            elif axis == 'y':
                for i in _iterable: self.points[i].negy()
            elif axis == 'xy':
            ...
            if axis == 'x':
                for i in indices: self.locx[i] *= -1
            elif axis == 'y':
                for i in indices: self.locy[i] *= -1
            elif axis == 'xy':
                self.neg(axis = 'x', indices = indices)
                self.neg(axis = 'y', indices = indices)
            self.__state_change = True
            self.clean(depth = 2)
        self.recompute_basics(npoints_flag = True, locx_locy_flag = False, centroid_flag = True,)
        self.__state_change = False
    # -----------------------------------------------
    def clean(self,
              depth = 0
        ...
        if depth == 0:
            ...
            if self.lean in ('no', 'low', 'medium'):
                self.recompute_from_mids()
                #=================================
                self.recompute_from_dist(method = 'points_normal', tolerance_distance = mulpoint2d.__POINT_TDIST__)
                #=================================
            if self.lean in ('high', 'veryhigh'):
                self.recompute_from_dist(method = 'coord_normal', tolerance_distance = mulpoint2d.__POINT_TDIST__)
        elif depth == 1:
            # clean at depth 0
    ...
    # -----------------------------------------------
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # END OF CKD TREE METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # START OF POINT ADD REMOVE METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def add(self,
            toadd = 'points',
            coord_list = [],
        ...
        '''
        mpobj.add(toadd = 'points', point_objects_list = [p1, p1, p1, p1, p1, p1, p1])
        ...
        if toadd == 'coord':
            for xy in coord_list:
                self.points.append(point2d(x = xy[0], y = xy[1]))
        if toadd == 'points':
    ...
    def rem(self,
        ...
        #m = mulpoint2d(point_objects = [p1, p2])
        #point_objects_list = [p2]
        #codist = 0.0
        if method == 'bynumber':
            if torem == 'points':
                # numbers corresponds to point numbers in thelist of self.points
                pass
            ...
            if torem == 'multi_points':
                point_objects_list = deque(pobj for mpobj in multi_point_objects_list for pobj in mpobj.points)
        ...
        #print(self.points)
        print('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        print(point_objects_list)
        print('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        __remove = [pobj.__eq__(_pobj, tdist = codist, use_global_tdist = False) for _pobj in point_objects_list for pobj in self.points]
        # Remove the point
        print('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        print(__remove)
        __remove.reverse()
        ...
        if self.__state_change:
            ...
            self.__state_change = False
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # END OF POINT ADD REMOVE METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # START OF DISTANCE AND ANGLE CALCULATION METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def distance(self,
                 x,
                 y
    ...
    def translate(self, delx, dely):
        ...
        if self.lean in ('no', 'low', 'medium'):
            if delx != 0:
                for pobj in self.points: pobj.x += delx
            if dely != 0:
                for pobj in self.points: pobj.y += dely
            self.recompute_basics(npoints_flag = False, locx_locy_flag = True, centroid_flag = True, )
        elif self.lean in ('high', 'veryhigh'):
            if delx != 0:
                self.locx += delx
            if dely != 0:
    ...
    def DG(self,
        ...
        if deformation == 'stretch':
            ...
            return np.array([[c, -s], [s, c]]) # Nothing but the rotation matrix
    ...
    def rotate1(self, x, y, θ):
        ...
        # Credit for above: https://danceswithcode.net/engineeringnotes/rotations_in_2d/rotations_in_2d.html
        _______________________________________________________
    ...
    def explode(self, x, y, factorx, factory):
        ...
        for count in range(self.npoints):
            self.points[count].x, self.points[count].y = dcosang[count], dsinang[count]
        if factorx==factory:
            centroid_flag = False
        self.recompute_basics(npoints_flag = False, locx_locy_flag = True, centroid_flag = centroid_flag, )
    # -----------------------------------------------
    def shear(self, deformation = 'shearx', shear = None):
        '''
        shear deform the multi-point object
        OPTIONS:
            1. shearx: shear along x - axis. A +ve shear tilts left vertical of fundamental
                        rectangle right, about bottom left corner
            2. sheary: shear along y - axis. A +ve shear tilts bottom horizontal of fundamental
                        rectangle up, about bottom left corner
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # START OF POINT REORDERING METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def merge(self,
              retain = 'first',
              ):
    ...
    def noise(self,
        ...
        if perturb_type == 'dgh':
            ...
            noise_x = ground + np.random.uniform(depth, height, (self.npoints,))
            noise_y = ground + np.random.uniform(depth, height, (self.npoints,))
            if self.lean in ('no', 'low', 'medium'):
                for count in range(self.npoints):
        ...
        if perturb_type == 'local_uniform':
            ...
            try:
                ...
                self.xpert += xpert
                self.ypert += ypert
            self.locx = np.array([_x+__x for (_x, __x) in zip(self.locx, self.xpert)])
            self.locy = np.array([_y+__y for (_y, __y) in zip(self.locy, self.ypert)])
    ...
    def relax(self,
        ...
        '''
        #def smoothLloyd2D(self, iterations=2, bounds=None, options='Qbb Qc Qx'):
    ...
    def recompute_from_mids(self):
        ...
        if self.lean in ('no', 'low', 'medium'):
            ...
            if len(mids) > len(list(set(mids))):
                mids = list(dict.fromkeys(mids)) # make the mids unique and unsorted
                self.unique_points_mids(mids)
                self.__state_change = True
            if self.__state_change:
                self.recompute_basics(npoints_flag = True, locx_locy_flag = True, centroid_flag = True )
            self.__state_change = False # CAUTION: DO NOT CHANGE EXECUTION ORDER
        elif self.lean in ('high', 'veryhigh'):
            pass
    # -----------------------------------------------
    ...
    # -----------------------------------------------
    def unique_points_dist(self, method = 'points_normal', tolerance_distance = 0.0):
        '''
        ...
        if method == 'points_normal':
            ...
            for count1, (_x, _y) in enumerate(zip(self.locx, self.locy), start = 0):
                if not repeated[count1]:
                    for count2, (__x, __y) in enumerate(zip(self.locx, self.locy), start = 0):
                        if count1 != count2:
            ...
            if True in repeated:
                ...
                self.locy = dcopy(__locy)# TODO: After testing, replace above and this line by the following line.
    ...
    def recompute_locx_locy(self):
        ...
        '''
        if self.lean in ('no', 'low', 'medium'):
            # TODO: Combine the following two lines into a single list comprehension
    ...
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # END OF RECOMPUTE METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # START OF BOUNDARY METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def qh(self,
           package = 'scipy',
    ...
    def hole(self):
        ...
        pass
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # END OF BOUNDARY METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # START OF SCALAR MAPPING METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    def mapsf_area(self):
        '''
    ...
    def perthist(self):
        ...
        ax.hist(self.xpert, bins = 25, label = 'x-pert', edgecolor = 'k', facecolor = 'teal', alpha = 0.5)
        ax.hist(self.ypert, bins = 25, label = 'y-pert', edgecolor = 'k', facecolor = 'orange', alpha = 0.5)
        ax.legend(loc='upper left', shadow=False, fontsize='small', edgecolor = 'k')
        #ax.set_xlim([-1, +1])
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    # END OF VIS METHODS
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
