...
#-----------------------------------------------------------------
xy = []
for domain_count in range(len(nx)):
    _previous_y_incr = 0
    x = np.linspace(xmin[domain_count],
                    xmax[domain_count],
                    nx[domain_count])
    xincr = x[1] - x[0]
    y = np.linspace(_previous_y_incr + ymin[domain_count],
                    _previous_y_incr + ymax[domain_count],
                    ny[domain_count])
    _previous_y_incr = y[1]-y[0]
    __xy = np.meshgrid(x, y)
    #===================================
    # Apply a transformtion for x and y if necessary
    # This will allow us to introduce gradients in grain structure
    #===================================
    x, y = __xy[0], __xy[1]
    ...
    x_flat = x.flatten()
    y_flat = y.flatten()
    for point_count in range(len(x_flat)):
        xy.append((x_flat[point_count], y_flat[point_count]))
#-----------------------------------------------------------------
...
xminmax = np.array([xmin, xmax]).flatten()
yminmax = np.array([ymin, ymax]).flatten()
